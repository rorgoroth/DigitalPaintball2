...
void M_Banner( char *name )
{
	...
	re.DrawPic( viddef.width / 2 - w*hudscale / 2, 
		viddef.height / 2 - 110*hudscale, name );
}
...
void M_DrawTextBox (int x, int y, int width, int lines)
{
	...
	while (width > 0)
	{
		...
		{
			cy += 8;
			M_DrawCharacter (cx, cy, 22); // jit, was 5
		}
...
void M_Main_Draw (void)
{
	...
	for ( i = 0; names[i] != 0; i++ )
	{
		...
		if (w > widest)
			widest = w;
		totalheight += (h + 12)*hudscale;
	}

	ystart = (viddef.height / 2 - 110*hudscale);
	xoffset = (viddef.width - widest*hudscale + 70*hudscale) / 2;

	for ( i = 0; names[i] != 0; i++ )
	{
		if ( i != m_main_cursor )
			re.DrawPic( xoffset, ystart + i * 40*hudscale + 13*hudscale, names[i] );
	}
	strcpy( litname, names[m_main_cursor] );
	strcat( litname, "_sel" );
	re.DrawPic( xoffset, ystart + m_main_cursor * 40*hudscale + 13*hudscale, litname );

	M_DrawCursor(xoffset - 25*hudscale, ystart + m_main_cursor * 40*hudscale + 11*hudscale, (int)(cls.realtime / 100)%NUM_CURSOR_FRAMES);

	re.DrawGetPicSize( &w, &h, "m_main_plaque" );
	re.DrawPic( xoffset - (30 + w)*hudscale, ystart, "m_main_plaque" );

	re.DrawPic( xoffset - (30 + w)*hudscale, ystart + h + 5, "m_main_logo" );
}
...
void M_Menu_Multiplayer_f( void )
{
	if (m_drawfunc == Multiplayer_MenuDraw) return;
...
=======================================================================
*/
char *bindnames[][2] = // jits, modified for paintball
{
{"+forward", 		"forward"},
{"+back", 			"backpedal"},
{"+speed", 			"walk"},
...
{"dropflag",		"drop flag"},
{"camera", 			"chasecam" },
{"noclip",			"noclip observer mode" },
{"jail",			"jail toggle" },
{ 0, 0 }
};
#define TOTAL_MENU_BINDS (sizeof(bindnames)/8-1)

int				keys_cursor;
static int		bind_grab;

static menuframework_s	s_keys_menu;
/* jits, easier to customize menus
...
*/
static menuaction_s		s_keys[TOTAL_MENU_BINDS]; // jits
...
static void M_UnbindCommand (char *command)
{
	...
	for (j=0 ; j<256 ; j++)
	{
		...
		if (!strcmp (b, command) ) // jitspoe, fix messagemode, messagemode2 thing
			Key_SetBinding (j, "");
	}
}
...
static void M_FindKeysForCommand (char *command, int *twokeys)
{
	...
	for (j=0 ; j<256 ; j++)
	{
		...
		if (!b)
			continue;
		//if (!strncmp (b, command, l) )
		if(!strcmp(b,command)) // jitspoe, minor bug fix where messagemode == messagemode2
		{
			twokeys[count] = j;
			count++;
			if (count == 2)
...
static void Keys_MenuInit( void )
{
	...
	s_keys_menu.x = viddef.width * 0.50;  // jitodo, move over for address book
	s_keys_menu.nitems = 0;
	s_keys_menu.cursordraw = KeyCursorDrawFunc;

	// ===
	// jitspoe, easier to manage bind menus.

	for(i=0;i<TOTAL_MENU_BINDS;i++)
	{
		s_keys[i].generic.type         = MTYPE_ACTION;
		s_keys[i].generic.flags        = QMF_GRAYED;
		s_keys[i].generic.x            = 0;
		s_keys[i].generic.y            = i*9;
		s_keys[i].generic.ownerdraw    = DrawKeyBindingFunc;
		s_keys[i].generic.localdata[0] = i;
		s_keys[i].generic.name         = bindnames[i][1];

		Menu_AddItem( &s_keys_menu, ( void * ) &s_keys[i] );
	}
/*
...
void M_Menu_Keys_f (void)
{
	if (m_drawfunc == Keys_MenuDraw) return;
	Keys_MenuInit();
...
static void UpdateSoundQualityFunc( void *unused )
{
	...
	{
		Cvar_SetValue( "s_khz", 44 ); // jit, was 22
...
void M_Menu_Options_f (void)
{
	if (m_drawfunc == Options_MenuDraw) return;
	Options_MenuInit();
...
void M_Menu_Video_f (void)
{
	if (m_drawfunc == VID_MenuDraw) return;
	VID_MenuInit();
...
void M_Menu_Credits_f( void )
{
	...
	if (m_drawfunc == M_Credits_MenuDraw) return;
...
void Game_MenuInit( void )
{
	...
	// jitodo, finish game menu:
	s_easy_game_action.generic.type	= MTYPE_ACTION;
	s_easy_game_action.generic.flags  = QMF_LEFT_JUSTIFY;
	s_easy_game_action.generic.x		= 0;
	s_easy_game_action.generic.y		= 0;
	s_easy_game_action.generic.name	= "Under Construction";
	//s_easy_game_action.generic.callback = CreditsFunc;
/*
	s_easy_game_action.generic.type	= MTYPE_ACTION;
	s_easy_game_action.generic.flags  = QMF_LEFT_JUSTIFY;
	s_easy_game_action.generic.x		= 0;
...
*/
	...
	Menu_AddItem( &s_game_menu, ( void * ) &s_easy_game_action ); // jitemp
...
void M_Menu_Game_f (void)
{
	if (m_drawfunc == Game_MenuDraw) return;
	Game_MenuInit();
...
void M_Menu_LoadGame_f (void)
{
	if (m_drawfunc == LoadGame_MenuDraw) return;
	LoadGame_MenuInit();
...
void M_Menu_SaveGame_f (void)
{
	...
	if (m_drawfunc == SaveGame_MenuDraw) return;
	SaveGame_MenuInit();
...
void JoinServer_MenuInit( void )
{
	...
	s_joinserver_address_book_action.generic.name	= "address book/download list";
...
void M_Menu_JoinServer_f (void)
{
	if (m_drawfunc == JoinServer_MenuDraw) return;
	JoinServer_MenuInit();
...
void RulesChangeFunc ( void *self )
{
	...
	{
		s_maxclients_field.generic.statusbar = "8 maximum for cooperative";
		if (atoi(s_maxclients_field.buffer) > 8)
...
//	strcpy( startmap, strchr( mapnames[s_startmap_list.curvalue], '\n' ) + 1 );
	strcpy(startmap, mapnames[s_startmap_list.curvalue]); // jit -- (list maps by .bsp files)
...
//	FILE *fp;
	...
	* jit -- let's check the bsp files instead
...
*/
	// ===
	// jit -- load map list from actual files, rather than a list in maps.lst
	char **bspfiles;

	extern char **FS_ListFiles( char *, int *, unsigned, unsigned );
	static void FreeFileList( char **list, int n );

	bspfiles = FS_ListFiles( BASEDIRNAME "/maps/*.bsp", &nummaps, 0, SFF_SUBDIR | SFF_HIDDEN | SFF_SYSTEM );
	
	mapnames = malloc(sizeof(char*) * (nummaps + 1));
	memset(mapnames, 0, sizeof(char*) * (nummaps + 1));

	// strip away all the filename junk -- we just want the map name
	for(i=0; i<nummaps - 1; i++)
	{
		scratch = bspfiles[i] + strlen(BASEDIRNAME)+6;
		s = strstr(scratch,".bsp");
		*s = 0;
		mapnames[i] = malloc(strlen(scratch) + 1);
		strcpy(mapnames[i], scratch);
	}

	FreeFileList(bspfiles, nummaps);
	// ===
...
//*/
	...
	s_fraglimit_field.generic.name = "score limit"; // jit
	...
	*/
	...
	if ( Cvar_VariableValue( "maxclients" ) == 1 )
		strcpy( s_maxclients_field.buffer, "16" );
	else 
	...
	s_hostname_field.length = 64;
	s_hostname_field.visible_length = 32;
	strcpy( s_hostname_field.buffer, Cvar_VariableString("hostname") );

	s_startserver_dmoptions_action.generic.type = MTYPE_ACTION;
	s_startserver_dmoptions_action.generic.name	= " deathmatch flags"; // jitodo, advanced
...
//	Menu_AddItem( &s_startserver_menu, &s_rules_box );
	...
	//Menu_AddItem( &s_startserver_menu, &s_startserver_dmoptions_action ); jit, we don't use deathmatch flags
...
void M_Menu_StartServer_f (void)
{
	if (m_drawfunc == StartServer_MenuDraw) return;
...
void M_Menu_DMOptions_f (void)
{
	if (m_drawfunc == DMOptions_MenuDraw) return;
	DMOptions_MenuInit();
...
void M_Menu_DownloadOptions_f (void)
{
	if (m_drawfunc == DownloadOptions_MenuDraw) return;
...
=============================================================================
*/
#define NUM_ADDRESSBOOK_ENTRIES 32	// ACT - We want more than 9 servers, COME ON!

static menuframework_s	s_addressbook_menu;
static menufield_s		s_addressbook_fields[NUM_ADDRESSBOOK_ENTRIES];
static menuaction_s		s_addressbook_getlist;	// ACT - button to get the list
...
extern cvar_t *serverlist; // jitserverlist

static void DownloadServerList( void *self )
{
	...
	//if (0 == NET_TCPConnect(serverListSocket,"www.planetquake.com",80)) // jitodo, make a cvar for this
	// ==[
	// jitserverlist:
	s = strstr(serverlist->string, "/");
	if(s)
		*s = 0;
	if(!NET_TCPConnect(serverListSocket, serverlist->string, 80))
		return;	// Couldn't connect
	// ]==
	
	// We're connected! Lets ask for the list
	{
		//char *msg = "GET /digitalpaint/servers.txt HTTP/1.0\n\n";
		char msg[256];
		int len, bytes_sent;
	...
	// We've sent our request... Lets read in what they've got for us
	{
		...
		while (current < buffer + numread) {
			...
			*current = 0;													// NULL terminate the string
			strcpy(s_addressbook_fields[WhichOn++].buffer,found);			// Copy the entire line
...
///* ACT
	s_addressbook_getlist.generic.type = MTYPE_ACTION;
	s_addressbook_getlist.generic.name	= "download server list";
	s_addressbook_getlist.generic.flags	= QMF_LEFT_JUSTIFY;
	s_addressbook_getlist.generic.x	= 24*hudscale;
	s_addressbook_getlist.generic.y	= 0;
	s_addressbook_getlist.generic.callback = DownloadServerList;
	s_addressbook_getlist.generic.statusbar = "get servers.txt from game host";
// ACT  */

	s_addressbook_menu.x = viddef.width / 2 - 142*hudscale;
	s_addressbook_menu.y = viddef.height / 2 - 58*hudscale;
	...
	for ( i = 0; i < NUM_ADDRESSBOOK_ENTRIES; i++ )
	{
		...
		s_addressbook_fields[i].generic.y		= i * 18*hudscale + 18; // jitspoe, make space for download option
	...
	Menu_AddItem( &s_addressbook_menu, &s_addressbook_getlist ); // ACT [it doesnt want to work unless it goes last...]
}
...
void M_Menu_AddressBook_f(void)
{
	if (m_drawfunc == AddressBook_MenuDraw) return;
...
qboolean PlayerConfig_MenuInit( void )
{
	...
	else
	{
		...
		strcpy( currentskin, "pb2b" ); // jit, default to paintball skin
	}
...
void PlayerConfig_MenuDraw( void )
{
	...
	if ( s_pmi[s_player_model_box.curvalue].skindisplaynames )
	{
		...
		M_DrawTextBox( ( refdef.x ) * ( 320.0F*hudscale / viddef.width ) - 8*hudscale, ( viddef.height / 2 ) * ( 240.0F*hudscale / viddef.height) - 77*hudscale, refdef.width / (8*hudscale), refdef.height / (8*hudscale) );
		refdef.height += 4*hudscale;
		...
		re.DrawPic( s_player_config_menu.x - 40*hudscale, refdef.y, scratch );
	}
}
...
void M_Menu_PlayerConfig_f (void)
{
	if (m_drawfunc == PlayerConfig_MenuDraw) return;
...
void M_Menu_Gallery_f( void )
{
	...
	if (m_drawfunc == Gallery_MenuDraw) return;
...
void M_Quit_Draw (void)
{
	...
	re.DrawPic ( (int)((viddef.width-w)*0.5), (int)((viddef.height-h)*0.5), "quit"); // jit, kill warning
}
*/

void M_Menu_Quit_f (void)
{
	CL_Quit_f (); // jit, just quit
	/*
	if (m_drawfunc == M_Quit_Draw) return;
